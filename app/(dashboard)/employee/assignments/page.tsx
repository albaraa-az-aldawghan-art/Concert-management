...
import { SearchBox, DateFilterBar, Pagination, matchesDate, emptyDateFilter, DateFilterState, tsToDateStr, compareDateValues } from "@/components/ui/list-filters";
...
export default function EmployeeAssignmentsPage() {
  ...
  if (loading) {
    return <LoadingState label="جارٍ تحميل حفلات الموظفين..." />;
  }
  ...
  return (
    <PageShell>
      <PageHeader title={isAdmin ? "حفلات الموظفين" : "حفلاتي"} icon={UsersRound}
        description={`${filtered.length} حفلة · افتح الحفلة لعرض المواد والطعام والتفاصيل`} />
      ...
      {filtered.length === 0 ? (
        <Card><EmptyState icon={Music} title={summaries.length === 0 ? "لا توجد حفلات مسندة إليك" : "لا توجد نتائج مطابقة"}
          description={summaries.length > 0 ? "غيّر البحث أو فلتر التاريخ." : undefined} /></Card>
      ) : (
        <>
